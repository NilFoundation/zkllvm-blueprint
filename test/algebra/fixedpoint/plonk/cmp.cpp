...
template<typename FixedType>
void test_fixedpoint_cmp_extended(FixedType input1, FixedType input2) {
    using BlueprintFieldType = typename FixedType::field_type;
    ...
    using component_type =
        blueprint::components::fix_cmp_extended<ArithmetizationType,
                                                BlueprintFieldType,
    ...
    auto result_check = [&expected_res_less,
        ...
        auto real_res_greater = var_value(assignment, real_res.gt) == 1;
        auto real_res_equal = var_value(assignment, real_res.eq) == 1;
        auto real_res_geq = var_value(assignment, real_res.geq) == 1;
        auto real_res_leq = var_value(assignment, real_res.leq) == 1;
        auto real_res_neq = var_value(assignment, real_res.neq) == 1;

#ifdef BLUEPRINT_PLONK_PROFILING_ENABLED
        std::cout << "fixed_point cmp_extended test: "
                  << "\n";
        std::cout << "input_f  :" << input1.to_double() << " " << input2.to_double() << "\n";
        std::cout << "input    : " << input1.get_value().data << " " << input2.get_value().data << "\n";
        std::cout << "expected<   : " << expected_res_less_f << "\n";
        ...
        std::cout << "expected=   : " << expected_res_equal_f << "\n";
        std::cout << "real=       : " << real_res_equal << "\n";
        std::cout << "expected!=  : " << expected_res_neq_f << "\n";
        std::cout << "real!=      : " << real_res_neq << "\n";
        std::cout << "expected<=  : " << expected_res_leq_f << "\n";
        std::cout << "real<=      : " << real_res_leq << "\n";
        std::cout << "expected>=  : " << expected_res_geq_f << "\n";
        std::cout << "real>=      : " << real_res_geq << "\n\n";
#endif
        if ((expected_res_less_f != real_res_less) || (expected_res_less != real_res_less)) {
            std::cout << "expected<         : " << expected_res_less << "\n";
            std::cout << "real<             : " << real_res_less << "\n";
        ...
        if ((expected_res_leq_f != real_res_leq) || (expected_res_leq != real_res_leq)) {
            std::cout << "expected<=        : " << expected_res_leq << "\n";
            std::cout << "real<=            : " << real_res_leq << "\n";
            std::cout << "expected<= (float): " << expected_res_leq_f << "\n\n";
            abort();
        }
        if ((expected_res_geq_f != real_res_geq) || (expected_res_geq != real_res_geq)) {
            std::cout << "expected>=        : " << expected_res_geq << "\n";
            std::cout << "real>=            : " << real_res_geq << "\n";
            std::cout << "expected>= (float): " << expected_res_geq_f << "\n\n";
            abort();
        }
        if ((expected_res_neq_f != real_res_neq) || (expected_res_neq != real_res_neq)) {
            std::cout << "expected!=        : " << expected_res_neq << "\n";
            std::cout << "real!=            : " << real_res_neq << "\n";
            std::cout << "expected!= (float): " << expected_res_neq_f << "\n\n";
            abort();
        }

        BLUEPRINT_RELEASE_ASSERT((uint8_t)real_res_equal + (uint8_t)real_res_greater + (uint8_t)real_res_less == 1);
        BLUEPRINT_ASSERT(real_res_eq != real_res_neq);
        BLUEPRINT_ASSERT(real_res_geq != real_res_less);
        BLUEPRINT_ASSERT(real_res_leq != real_res_greater);
    };
...
template<typename FixedType>
void test_fixedpoint_min(FixedType input1, FixedType input2) {
    using BlueprintFieldType = typename FixedType::field_type;
    constexpr std::size_t WitnessColumns = 5 + FixedType::M_1 + FixedType::M_2;
    constexpr std::size_t PublicInputColumns = 1;
    constexpr std::size_t ConstantColumns = 2;
    ...
    using component_type = blueprint::components::
        fix_min<ArithmetizationType, BlueprintFieldType, nil::blueprint::basic_non_native_policy<BlueprintFieldType>>;
...
void test_components_on_random_data(RngType &rng) {
    ...
    test_fixedpoint_cmp_extended<FixedType>(x, y);
    test_fixedpoint_max<FixedType>(x, y);
    test_fixedpoint_min<FixedType>(x, y);
