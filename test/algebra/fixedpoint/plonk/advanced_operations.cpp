...
template<typename FixedType>
void test_fixedpoint_log_ranged(FixedType input) {
    using BlueprintFieldType = typename FixedType::field_type;
    constexpr std::size_t WitnessColumns = 10;
    constexpr std::size_t PublicInputColumns = 1;
    constexpr std::size_t ConstantColumns = 10;
    ...
    using component_type =
        blueprint::components::fix_log_ranged<ArithmetizationType,
                                              BlueprintFieldType,
                                              nil::blueprint::basic_non_native_policy<BlueprintFieldType>>;
...
void test_components_on_random_data(RngType &rng) {
    ...
    if (x.get_value() >= 0) {
        ...
        if (x.get_value() > 0) {
            ...
            test_fixedpoint_log_ranged<FixedType>(x);
        }
    }
    ...
    if (y.get_value() >= 0) {
        ...
        if (y.get_value() > 0) {
            ...
            test_fixedpoint_log_ranged<FixedType>(y);
        }
    }
}
...
void field_operations_test() {
    if (FixedType::M_2 == 2) {
        ...
        test_fixedpoint_sqrt_floor<FixedType>(FixedType(487342407766168868ULL, 32));
    }

    test_fixedpoint_sqrt<FixedType>(FixedType::max());
    test_fixedpoint_sqrt_floor<FixedType>(FixedType::max());
    test_fixedpoint_log<FixedType>(FixedType::max());
    test_fixedpoint_log_ranged<FixedType>(FixedType::max());
