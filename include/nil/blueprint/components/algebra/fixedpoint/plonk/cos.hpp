        ...
        namespace components {
            ...
            public:
                ...
                var_positions get_var_pos(const int64_t start_row_index) const {
                    ...
                    //              (11 col(s), 3 row(s) for 32.16 fixed point)
...
// Allows disabling lookup tables for faster testing
            ...
            typename plonk_fixedpoint_cos<BlueprintFieldType, ArithmetizationParams>::result_type generate_assignments(
                ...
                if (m1 == 2) {
                    ...
                    for (int64_t i = 0; i < m2 + 1; i++) {    // copy decomposition of z to cos
                    ...
                    FixedPointHelper<BlueprintFieldType>::decompose(x_val, x0_val);
                }
            ...
            std::vector<crypto3::zk::snark::plonk_constraint<BlueprintFieldType>> get_constraints(
                ...
                // we don't care about the sign of x, only applies if m1=1 (decomp of x mod 2*pi is constrained in rem)
                auto constraint_1 = (x - x0) * (x + x0);
